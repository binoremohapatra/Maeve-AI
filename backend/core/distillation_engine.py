...
def _keyword_extract_memories(messages: List[Dict]) -> List[Dict]:
    ...
    patterns = {
        "LOVE": [r"i love you", r"you're the only one", r"my heart belongs to"],
        "ABUSE": [r"you're worthless", r"i hate you", r"stupid idiot", r"garbage"],
        "SEXUAL": [r"i want you so badly", r"let's make love", r"need you right now"],
        "BETRAYAL": [r"talking to another", r"she's just a friend", r"i was with someone else"],
        "APOLOGY": [r"i'm so sorry", r"please forgive me", r"i apologize"]
    }
    ...
    for msg in messages:
        ...
        for category, pattern_list in patterns.items():
            for pattern in pattern_list:
                if hasattr(pattern, 'search') and pattern.search(content) and f"{category}_{timestamp}" not in seen_patterns:
                    memory = {
                        "type": category,
